...
uint16_t INA219::read_register(uint8_t register_address){
	...
	if (i2c_write_blocking(i2c0, __ADDRESS, buf, 1, true) != 1) {
		printf("\n-Failed to set register.");
	}
	sleep_us(1000);
	num_bytes_read = i2c_read_blocking(i2c0, __ADDRESS, buf, 2, false);
	if (num_bytes_read != 2) {
...
void INA219::write_register(uint8_t register_address, uint16_t register_value){
    ...
    if (i2c_write_blocking(i2c0, __ADDRESS, buf, 3, false) != 3)
	{
...
void INA219::configure(int voltage_range, int gain, int bus_adc, int shunt_adc){
	...
	if (voltage_range > len-1) {
		printf("Invalid voltage range, must be one of: RANGE_16V, RANGE_32");
	}
